...
void define_data(IloEnv env)//数据初始化,对全局变量进行赋值
{
	...
	{
		Unit[i]=IloNumArray(env,12);
		//12条信息:结点、Pmin、Pmax、MinOn、MinOff、RU、RD、Startup Cost、c、b、a、price;
...
/***************************子问题建议行为约束*************************************/
				...
				for(IloInt i=0; i<NG; ++i)//绝对值转化为两个约束
				{
					...
					for(IloInt t=0; t<NT; ++t)
					{
						ramp1[i][t]=IloRange::IloRange(env,-IloInfinity,PS[i][t]-S5S[i][t],detaa[i]+P1[i][t],"ramp1");
						ramp2[i][t]=IloRange::IloRange(env,-IloInfinity,-PS[i][t]-S5S[i][t],detaa[i]-P1[i][t],"ramp2");
