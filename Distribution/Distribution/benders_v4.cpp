...
const IloNum Cost_Modify=1;//暂时无用
...
/*************************************************安全约束*********************************************/
		...
		for(int t=0;t<NT;++t)//校验所有时段的所有支路的潮流
		{
			...
			IloNumVarArray PL(env,Branch,-IloInfinity,IloInfinity,ILOFLOAT);//各条支路的潮流
			...
			for(IloInt k=0; k<Node-1; ++k)//校验了所有结点的潮流
			{
				...
				if(i<NG)
					Master_Model.add(P_dual[k]==P[i][t]);	
				else
					Master_Model.add(P_dual[k]==0);
			...
			{
				Master_Model.add(PL[h]==(Theta[(IloInt)Info_Branch[h][0]-1]-Theta[(IloInt)Info_Branch[h][1]-1])/Info_Branch[h][3]);
				Master_Model.add(PL[h]<=Info_Branch[h][4]);
				Master_Model.add(-PL[h]<=Info_Branch[h][4]);
			}
			Psp.end();
			Theta.end();
...
/***************************子问题建议行为约束*************************************/
				...
				for(IloInt i=0; i<NG; ++i)//绝对值转化为两个约束
				{
					...
					for(IloInt t=0; t<NT; ++t)
					{
						ramp1[i][t]=IloRange::IloRange(env,-IloInfinity,PS[i][t]-S3S[i][t],detaa+P1[i][t],"ramp1");
						ramp2[i][t]=IloRange::IloRange(env,-IloInfinity,-PS[i][t]-S3S[i][t],detaa-P1[i][t],"ramp2");
						//以上两个都是NG*NT维的约束
...
/*************************************安全约束************************************/				
				for(int t=0;t<NT;++t)
				{
					...
					for(IloInt k=0; k<Node-1; ++k)//校验了所有支路的潮流
					{
						...
						if(i<NG)
							Sub_Model.add(P_dual[k]==PS[i][t]);	
						else
							Sub_Model.add(P_dual[k]==0);
					...
					for(IloInt h=0; h<Branch; ++h)
					{
						Sub_Model.add(PL[h]==(Theta[(IloInt)Info_Branch[h][0]-1]-Theta[(IloInt)Info_Branch[h][1]-1])/Info_Branch[h][3]);
						Sub_Model.add(PL[h]<=Info_Branch[h][4]);
						Sub_Model.add(-PL[h]<=Info_Branch[h][4]);
					}
